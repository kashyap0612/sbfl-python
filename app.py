...
if run_button:
    ...
    test_funcs = {name: func for name, func in inspect.getmembers(user_tests, inspect.isfunction) if name.startswith("test_")}
    if not test_funcs:
    ...
    for ln, info in top:
        st.write(f"Line {ln}: Ensemble={info['ensemble']:.3f}  | Tarantula={info['tarantula']:.3f}  | Ochiai={info['ochiai']:.3f}  | DStar={info['dstar']:.3f}")
